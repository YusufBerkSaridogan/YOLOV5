...
while True:
    ...
    # Draw coordinates above the lines
        ...
        for index, row in results.pandas().xyxy[0].iterrows():
            ...
            if 'person' in label:
               #cv2.putText(frame, str(confidence), (x1, y1), cv2.QT_FONT_NORMAL, 0.45, (255,0,0), 1)
        ...
        for box_id in boxes_ids:
                ...
                cv2.putText(frame, ("ID:"+str(id)), (x, y), cv2.QT_FONT_NORMAL, 0.45, (255,0,0), 1)

                bot_mid_point = int(x + (w - x) / 2), int(h)
                
                #Calculate the if the person is in the area
